...
const insertMovieActors = (
  ...
  actors.map(actor => `(
    '${(movieId)}',
    '${(actorRows.find(it => it.full_name === actor)!.id)}')`).join(","));
};
...
const insertMovieDirectors = (
  ...
  directors.map(director => `(
    '${(movieId)}',
    '${(directorRows.find(it => it.full_name === director)!.id)}')`).join(","));
};
...
const insertMovieProductionCompanies = (
  ...
  productionCompanies.map(company => `(
    '${(movieId)}',
    '${(productionCompanyRows.find(it => it.company_name === company)!.id)}')`).join(","));
};
